// This file is auto-generated by @hey-api/openapi-ts

import { z } from 'zod';

export const zError = z.object({
    error: z.string(),
    message: z.string()
});

export const zProgram = z.object({
    id: z.int(),
    name: z.string(),
    versions: z.array(z.string())
});

export const zUser = z.object({
    id: z.string(),
    email: z.email(),
    name: z.string(),
    role: z.enum([
        'user',
        'editor',
        'admin'
    ]),
    active: z.union([z.literal(0), z.literal(1)]),
    verified: z.union([z.literal(0), z.literal(1)]),
    verified_at: z.optional(z.union([
        z.iso.datetime(),
        z.null()
    ])),
    verified_until: z.optional(z.union([
        z.iso.datetime(),
        z.null()
    ])),
    programid: z.int(),
    created_at: z.iso.datetime(),
    updated_at: z.iso.datetime()
});

export const zUserRegister = z.object({
    email: z.email(),
    name: z.string().min(1),
    password: z.string().min(16),
    programid: z.int()
});

export const zUserLogin = z.object({
    email: z.email(),
    password: z.string()
});

export const zSession = z.object({
    id: z.string(),
    userid: z.string(),
    created_at: z.iso.datetime(),
    last_seen: z.iso.datetime(),
    expires_at: z.iso.datetime()
});

export const zEvent = z.object({
    id: z.int(),
    title: z.string(),
    cover_src: z.optional(z.string()),
    created_at: z.optional(z.iso.datetime())
});

export const zMedia = z.object({
    id: z.string(),
    event_id: z.int(),
    src: z.string(),
    thumb: z.optional(z.string())
});

export const zCsrfHeader = z.string();

export const zGetAuthCsrfData = z.object({
    body: z.optional(z.never()),
    path: z.optional(z.never()),
    query: z.optional(z.never())
});

/**
 * CSRF token issued
 */
export const zGetAuthCsrfResponse = z.object({
    csrf: z.string()
});

export const zPostAuthRegisterData = z.object({
    body: zUserRegister,
    path: z.optional(z.never()),
    query: z.optional(z.never())
});

/**
 * User created
 */
export const zPostAuthRegisterResponse = zUser;

export const zGetAuthVerifyData = z.object({
    body: z.optional(z.never()),
    path: z.optional(z.never()),
    query: z.object({
        token: z.string()
    })
});

/**
 * Email verified successfully
 */
export const zGetAuthVerifyResponse = z.object({
    message: z.optional(z.string())
});

export const zPostAuthLoginData = z.object({
    body: zUserLogin,
    path: z.optional(z.never()),
    query: z.optional(z.never())
});

/**
 * Logged in
 */
export const zPostAuthLoginResponse = zUser;

export const zPostAuthLogoutData = z.object({
    body: z.optional(z.never()),
    path: z.optional(z.never()),
    query: z.optional(z.never()),
    headers: z.object({
        'X-CSRF-Token': z.string()
    })
});

/**
 * Logged out
 */
export const zPostAuthLogoutResponse = z.void();

export const zGetAuthMeData = z.object({
    body: z.optional(z.never()),
    path: z.optional(z.never()),
    query: z.optional(z.never())
});

/**
 * Current user
 */
export const zGetAuthMeResponse = zUser;

export const zGetUsersIdData = z.object({
    body: z.optional(z.never()),
    path: z.object({
        id: z.string()
    }),
    query: z.optional(z.never())
});

/**
 * User
 */
export const zGetUsersIdResponse = zUser;

export const zGetUsersData = z.object({
    body: z.optional(z.never()),
    path: z.optional(z.never()),
    query: z.optional(z.object({
        limit: z.optional(z.int().gte(1).lte(256)).default(32),
        offset: z.optional(z.int().gte(0)).default(0)
    }))
});

/**
 * List of users
 */
export const zGetUsersResponse = z.array(zUser);

export const zGetProgramsData = z.object({
    body: z.optional(z.never()),
    path: z.optional(z.never()),
    query: z.optional(z.never())
});

/**
 * List of programs
 */
export const zGetProgramsResponse = z.array(zProgram);

export const zGetProgramsIdData = z.object({
    body: z.optional(z.never()),
    path: z.object({
        id: z.int()
    }),
    query: z.optional(z.never())
});

/**
 * Program
 */
export const zGetProgramsIdResponse = zProgram;

export const zGetMediaEventsData = z.object({
    body: z.optional(z.never()),
    path: z.optional(z.never()),
    query: z.optional(z.never())
});

/**
 * List of events
 */
export const zGetMediaEventsResponse = z.array(zEvent);

export const zPostMediaEventsData = z.object({
    body: z.object({
        title: z.string()
    }),
    path: z.optional(z.never()),
    query: z.optional(z.never()),
    headers: z.object({
        'X-CSRF-Token': z.string()
    })
});

/**
 * Event created
 */
export const zPostMediaEventsResponse = zEvent;

export const zGetMediaEventImagesData = z.object({
    body: z.optional(z.never()),
    path: z.object({
        id: z.int()
    }),
    query: z.optional(z.never())
});

/**
 * List of images
 */
export const zGetMediaEventImagesResponse = z.array(zMedia);

export const zPostMediaEventImagesData = z.object({
    body: z.optional(z.object({
        images: z.optional(z.array(z.string())),
        cover: z.optional(z.boolean()).default(false)
    })),
    path: z.object({
        id: z.int()
    }),
    query: z.optional(z.never()),
    headers: z.object({
        'X-CSRF-Token': z.string()
    })
});

/**
 * Images uploaded
 */
export const zPostMediaEventImagesResponse = z.array(zMedia);

export const zGetMediaImageData = z.object({
    body: z.optional(z.never()),
    path: z.object({
        id: z.string()
    }),
    query: z.optional(z.never())
});

/**
 * Image content
 */
export const zGetMediaImageResponse = z.string();

export const zDeleteMediaImageData = z.object({
    body: z.optional(z.never()),
    path: z.object({
        id: z.string()
    }),
    query: z.optional(z.never()),
    headers: z.object({
        'X-CSRF-Token': z.string()
    })
});

/**
 * Deleted
 */
export const zDeleteMediaImageResponse = z.void();
